 ...
 * 策略模式
 *
 * 定义：策略模式是指对一系列的算法定义，并将每一个算法封装起来，
 ...
 *
 * 优点：① 策略模式提供了管理相关的算法族的办法。策略类的等级结构定义了一个算法或行为族。
 ...
 * 决定使用哪一种算法或采取哪一种行为的逻辑就和算法或行为的逻辑混合在一起，
 ...
 *
 * 缺点：①客户端必须知道所有的策略类，并自行决定使用哪一个策略类。这就意味着客户端必须理解这些算法的区别，
 ...
 *
 * 应用场景：① 多个类只区别在表现行为不同，可以使用Strategy模式，在运行时动态选择具体要执行的行为。
...
public class Strategy {
    ...
    public static void main(String[] args) {
        ...
        if (scanner.hasNextInt()) {
            context = new CashContext(scanner.nextInt());
        } else {
            System.out.println("请输入正确的数字！");
            return;
        }
        System.out.println("请输入收款金额数字");
        if (scanner.hasNextDouble()) {
            result = context.getResult(scanner.nextDouble());
            System.out.println("最终收款金额：" + result);
        } else {
            System.out.println("请输入正确的数字！");
            return;
        }
